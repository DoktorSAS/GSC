...
onStartGameType()
{
	...
	bots_team = getDvar("bots_team");

	game["attackers"] = getOtherTeam(bots_team);
	game["defenders"] = bots_team;
...
getSpawnPoint()
{
	if(self.pers["team"] == getDvar("bots_team"))
...
onDeadEvent( team )
{
    ...
    playersTeam = getOtherTeam(getDvar("bots_team"));

    // Check the team for no lives limits on players team
	if(team == playersTeam){
		if(getDvarInt("sd_allowattackersrevive"))
            self.pers["lives"]++;
			return;
	}
...
bombs()
{
	...
	for ( index = 0; index < bombZones.size; index++ )
	{
        ...
        bombZone maps\mp\gametypes\_gameobjects::setUseText( "You can't plant the bomb");
	    bombZone maps\mp\gametypes\_gameobjects::setUseHintText( "You can't plant the bomb" );
		if ( !level.multiBomb )
...
onBeginUse( player ) 
{
    // Disable plant/defuse event event
...
bombPlanted( destroyedObj, player )
{
	...
	else
	{
		...
		// Commented because this line of code generate issues with the plutonium gsc compiler. It will
		// print some message to the console that can make the server lag if keep printing.
	...
	*/
	...
	origin = level.bombZones[1].bombDefuseTrig.origin - (0,0,-10000); // Move the icon in the bomb position
	if(isDefined(level.sdBombModel))
		level.sdBombModel hide(); // Hide the bombmodel
	...
	rot = randomfloat(360);
	explosionEffect = spawnFx( level._effect["bombexplosion"], explosionOrigin, (0,0,1), (cos(rot),sin(rot),0) );
